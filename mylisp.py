...
def eval(x, env=global_env):
    ...
    elif x[0] == 'last':            # (last exp)
        (_, exp) = x
        return eval(exp, env)[-1]
    elif x[0] == 'reverse':         # (reverse exp)
        (_, exp) = x
        return eval(exp, env)[::-1]
    elif x[0] == 'length':         # (length exp)
        (_, exp) = x
        return len(eval(exp, env))
    elif x[0] == 'append':         # (append exp exp2)
        (_, exp1, exp2) = x
        res = eval(exp1, env)
        res.append(eval(exp2, env))
        return res
...
def load(filename):
    """
    Load the mylisp program in filename, execute it, and start the
    repl.  If an error occurs, execution stops, and we are left in the
    repl.  Note that load copes with multi-line mylisp code by
